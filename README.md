...
`.gitattributes`에 Smart Merge 설정은 잡혀있는데, **각자 로컬에서 한 번** mergetool 경로를 git에 알려줘야 실제로 동작해.
...
> `<Unity버전>` 자리는 본인이 깐 Unity 6 LTS 정확한 버전 이름으로 바꿔야 함. (예: `6000.0.30f1`)

---

### Step 9. Meta XR SDK 설치 (B 담당이면 필수, 나머지는 권장)

Meta XR All-in-One SDK는 라이선스 동의가 필요해서 `manifest.json`에 미리 못 박아둘 수 없음.

1. Unity Editor → `Window → Package Manager`
2. 좌상단 `+` 버튼 → `Add package by name...`
3. 입력: `com.meta.xr.sdk.all` → Add
4. (또는) Unity Asset Store에서 "Meta XR All-in-One SDK" 검색 → Import

> 기본 Unity XR Interaction Toolkit / Oculus XR Plugin은 이미 `manifest.json`에 들어가 있어서 자동 설치돼.
...
> ⚠️ MainScene은 `CODEOWNERS`로 D 승인 없이는 머지 못 하게 보호돼 있음.
...
> 채우고 나면 `.github/CODEOWNERS`의 `@팀원X-깃허브아이디` 자리도 실제 아이디로 바꿔주기.
